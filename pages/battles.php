...
require_once dirname(__DIR__) . '/services/ChallengeService.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'challenge') {
    ...
    $useTechniques = isset($_POST['use_dao_techniques']) && $_POST['use_dao_techniques'] === '1';
    if ($defenderId > 0) {
        $challengeResult = $challengeService->createChallenge($userId, $defenderId, $useTechniques);
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'accept' && $challengeId > 0) {
        $result = $challengeService->acceptChallenge($challengeId, $userId, $useTechniques);
        if ($result['success']) {
                                ...
                                <?php if ($isDefender): ?>
                                    <div class="flex gap-2">
                                        <form method="POST" class="inline">
                                            ...
                                            <input type="hidden" name="use_dao_techniques" value="1">
                                            <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg text-white font-semibold transition-all">
                                                Accept With Techniques
                                            </button>
                                        </form>
                                        <form method="POST" class="inline">
                                            <input type="hidden" name="action" value="accept">
                                            <input type="hidden" name="challenge_id" value="<?php echo $challenge['id']; ?>">
                                            <input type="hidden" name="use_dao_techniques" value="0">
                                            <button type="submit" class="px-4 py-2 bg-cyan-700 hover:bg-cyan-800 rounded-lg text-white font-semibold transition-all">
                                                Accept Basic Combat
                                            </button>
                                        </form>
                                        <form method="POST" class="inline">
                    ...
                    <div class="bg-gray-800/90 backdrop-blur-lg border border-cyan-500/30 rounded-xl p-4">
                        ...
                        <?php if ($rateLimitStatus['can_challenge'] && $canPvP): ?>
                            <form method="POST" class="mt-3">
                                ...
                                <label class="flex items-center gap-2 text-xs text-gray-300 mb-3">
                                    <input type="checkbox" name="use_dao_techniques" value="1" checked class="rounded border-gray-600 bg-gray-900 text-red-500 focus:ring-red-500">
                                    Auto-use Dao techniques during combat
                                </label>
