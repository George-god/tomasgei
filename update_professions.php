...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['main_profession_id'])) {
    header('Location: professions.php?err=' . urlencode('Invalid request.'));
    exit;
}
...
if (!$r1['success']) {
    header('Location: professions.php?err=' . urlencode($r1['message'] ?? 'Failed.'));
    exit;
}
...
if ($secondaryId > 0) {
    ...
    if (!$r2['success']) {
        header('Location: professions.php?err=' . urlencode($r2['message'] ?? 'Failed to set secondary.'));
        exit;
    }
}

header('Location: professions.php?msg=' . urlencode('Professions updated.'));
exit;
