...
class EraService
{
    ...
    public function getHallOfLegends(int $limit = 10): array
    {
        try {
            ...
            // Top players by final rating across all eras
            $sql = "SELECT er.*, u.username, e.name as era_name, e.start_date, e.end_date
            ...
            // Top players by wins
            $sql = "SELECT er.*, u.username, e.name as era_name
                    FROM era_rankings er
                    INNER JOIN users u ON er.user_id = u.id
                    INNER JOIN eras e ON er.era_id = e.id
            ...
            // Top sects by final rating
            $sql = "SELECT esr.*, s.name as sect_name, e.name as era_name
            ...
            $stmt = $db->prepare($sql);
            $stmt->execute([$limit]);
            $topTerritorySects = $stmt->fetchAll();
