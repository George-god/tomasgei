...
class RankingService
{
    ...
    {
        try {
            $cacheKey = 'ranking:leaderboard:' . max(1, $limit) . ':' . max(0, $offset);
            return Cache::remember($cacheKey, 15, function () use ($limit, $offset): array {
                ...
                $sql = "SELECT u.id, u.username, u.realm_id, u.level, u.rating,
                        ...
                        ORDER BY u.rating DESC, u.wins DESC, u.id ASC
    ...
    public function getUserRank(int $userId): int
    {
        try {
            $db = Database::getConnection();
            $sql = "SELECT COUNT(*) + 1 AS rank_position
                    FROM users ranked
                    JOIN users current_user ON current_user.id = ?
                    WHERE ranked.rating > current_user.rating
                       OR (ranked.rating = current_user.rating AND ranked.wins > current_user.wins)";
            $stmt = $db->prepare($sql);
            $stmt->execute([$userId]);
            $result = $stmt->fetch();
            return (int)($result['rank_position'] ?? 0);
        } catch (PDOException $e) {
