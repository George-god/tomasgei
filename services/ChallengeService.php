...
class ChallengeService
{
     ...
     */
    public function createChallenge(int $challengerId, int $defenderId, bool $attackerUseTechniques = true): array
    {
        // Validate users
        ...
        try {
            ...
            $sql = "INSERT INTO battle_challenges (challenger_id, defender_id, attacker_use_techniques, expires_at) 
                    VALUES (:challenger_id, :defender_id, :attacker_use_techniques, :expires_at)";
            $stmt = $db->prepare($sql);
            $stmt->execute([
                ':challenger_id' => $challengerId,
                ':defender_id' => $defenderId,
                ':attacker_use_techniques' => $attackerUseTechniques ? 1 : 0,
     ...
     */
    public function acceptChallenge(int $challengeId, int $defenderId, bool $defenderUseTechniques = true): array
    {
        try {
            $db = Database::getConnection();
            $db->beginTransaction();
            ...
            $stmt = $db->prepare("UPDATE battle_challenges SET status = 'accepted', defender_use_techniques = ?, responded_at = NOW() WHERE id = ?");
            $stmt->execute([$defenderUseTechniques ? 1 : 0, $challengeId]);
            ...
            $battleResult = $battleService->simulateBattle($challengerId, $defenderId, [
                'attacker_use_techniques' => (bool)($challenge['attacker_use_techniques'] ?? true),
                'defender_use_techniques' => $defenderUseTechniques,
            ]);
