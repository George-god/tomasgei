...
<body class="bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div class="bg-gray-800/90 backdrop-blur border border-amber-500/30 rounded-xl p-6 mb-8">
            ...
            <form id="create-listing-form" method="POST" action="create_listing.php" class="flex flex-wrap gap-4 items-end">
                <div>
...
(function() {
    ...
    if (createForm) {
        createForm.addEventListener('submit', function(e) {
            ...
            fetch('create_listing.php', { method: 'POST', body: fd, credentials: 'same-origin' })
    ...
    document.querySelectorAll('.buy-listing-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            fetch('buy_listing.php', { method: 'POST', body: fd, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.success) {
                        if (row) row.remove();
    ...
    document.querySelectorAll('.cancel-listing-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            var fd = new FormData();
            fd.append('listing_id', id);
            fetch('cancel_listing.php', { method: 'POST', body: fd, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
                    if (data.success) {
                        if (row) row.remove();
