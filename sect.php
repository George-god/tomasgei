...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    } elseif ($action === 'promote_elder') {
        $memberUserId = (int)($_POST['user_id'] ?? 0);
        $result = $sectService->promoteToElder($userId, $memberUserId);
        $message = $result['success'] ? $result['message'] : null;
        $error = $result['success'] ? null : $result['message'];
    } elseif ($action === 'demote_elder') {
        $memberUserId = (int)($_POST['user_id'] ?? 0);
        $result = $sectService->demoteElder($userId, $memberUserId);
        $message = $result['success'] ? $result['message'] : null;
        $error = $result['success'] ? null : $result['message'];
...
$isLeader = $mySect && (string)($mySect['role'] ?? '') === 'leader';
$eldersCount = $mySect ? (int)array_reduce($members, function ($c, $m) { return $c + ((string)($m['role'] ?? '') === 'elder' ? 1 : 0); }, 0) : 0;
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div class="bg-gray-800/90 backdrop-blur border border-amber-500/30 rounded-xl p-6 mb-8">
            ...
            <p class="text-gray-400 text-sm mb-1">Tier: <strong class="text-white"><?php echo htmlspecialchars(ucfirst((string)$mySect['tier']), ENT_QUOTES, 'UTF-8'); ?></strong> · Sect EXP: <?php echo number_format((int)($mySect['sect_exp'] ?? 0)); ?></p>
            <p class="text-gray-500 text-xs mb-4">Your role: <strong><?php echo htmlspecialchars(ucfirst((string)$mySect['role'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></strong>. Bonuses: +<?php echo (int)round((float)($mySect['bonuses']['cultivation_speed'] ?? 0) * 100); ?>% cultivation, +<?php echo (int)round((float)($mySect['bonuses']['gold_gain'] ?? 0) * 100); ?>% gold<?php if ((float)($mySect['bonuses']['breakthrough'] ?? 0) > 0): ?>, +<?php echo (int)round((float)($mySect['bonuses']['breakthrough']) * 100); ?>% breakthrough<?php endif; ?>.</p>
            ...
            <ul class="space-y-2 mb-4">
                ...
                <li class="flex flex-wrap items-center justify-between gap-2 bg-gray-900/50 rounded-lg px-3 py-2">
                    ...
                    <span class="text-gray-400 text-sm"><?php echo htmlspecialchars(ucfirst((string)$m['role']), ENT_QUOTES, 'UTF-8'); ?></span>
                    ...
                    <?php if ($isLeader && (int)$m['user_id'] !== $userId): ?>
                        <?php if ((string)$m['role'] === 'disciple' && $eldersCount < 5): ?>
                        <form method="POST" class="inline">
                            <input type="hidden" name="action" value="promote_elder">
                            <input type="hidden" name="user_id" value="<?php echo (int)$m['user_id']; ?>">
                            <button type="submit" class="text-amber-400 hover:underline text-sm">Promote to elder</button>
                        </form>
                        <?php elseif ((string)$m['role'] === 'elder'): ?>
                        <form method="POST" class="inline">
                            <input type="hidden" name="action" value="demote_elder">
                            <input type="hidden" name="user_id" value="<?php echo (int)$m['user_id']; ?>">
    ...
    (function() {
        ...
        function fetchMessages(afterId) {
            var url = 'sect_chat_fetch.php' + (afterId ? '?last_message_id=' + afterId : '');
        ...
        form.addEventListener('submit', function(e) {
            ...
            fetch('sect_chat_send.php', { method: 'POST', body: fd, credentials: 'same-origin' })
                .then(function(r) { return r.json(); })
                .then(function(data) {
